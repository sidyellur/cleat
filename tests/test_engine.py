...
def test_query_responder_write_serialized_with_api_writes(bash_eng):
    ...
    t.start()
    # The responder is gated (issue #16): after the first prompt, outside
    # altscreen, it ignores query-looking bytes in command output. Pretend a
    # full-screen program owns the terminal so this one query IS answered -
    # what's under test here is the locking around the write, not the gate
    # (covered separately below). Resetting prompts_seen wouldn't do: the
    # next prompt's A mark usually arrives in the same read chunk as the
    # query bytes, and the gate is evaluated on post-chunk state.
